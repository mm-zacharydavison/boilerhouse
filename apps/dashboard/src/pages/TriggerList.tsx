...
function CreateTriggerModal({
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		try {
			...
			await api.createTrigger({ name, type, tenant, workload, config });
			onCreated();
			onClose();
		} catch (err) {
	...
	return (
		<div>
			...
			{data.length === 0 ? (
				...
				<DataTable headers={["Name", "Type", "Workload", "Tenant", "Status", "Last Invoked", "Created", "Actions"]}>
					{data.map((t) => (
						<DataRow key={t.id}>
							...
							<td className="px-4 py-3 text-muted text-xs">{timeAgo(t.createdAt)}</td>
							<td className="px-4 py-3">
								<div className="flex gap-2">
									<ActionButton
										label="test"
