...
export interface EnsureImageResult {
	/** The resolved image reference to use in the pod spec. */
	imageRef: string;
	/** Whether the image was built/loaded locally (requires imagePullPolicy: Never). */
	localBuild: boolean;
}
 ...
 */
export class MinikubeImageProvider {
	constructor(
